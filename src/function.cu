...
__global__ void layer_norm_backward_kernel(float* x1_grad, float* x1_data,
                                           float* x_grad, int N, index_t dim,
                                           index_t dsize, float ep);
...
void MatMulExp::forward_process(Tensor& x1, Tensor& x2, Tensor& x) {
  ...
  if (x1.device() == "cpu") {
    ...
    // std::cout << dim << " " << shape[shape.size() - 2] << " " << shape[shape.size() - 1] << " " << shape1[shape1.size() - 1] << std::endl;
...
void MatMulExp::lhs_grad_fn(TensorImplPtr x1, TensorImplPtr x2,
  ...
  //     std::cout << "before matmul backward has nan." << std::endl;
  //     exit(0);
  //   }
  // }
  // for (int i = 0; i < x1->dsize(); ++i) {
  //   if (std::isnan(x1->grad_[i])) {
  //     std::cout << "after matmul backward has nan." << std::endl;
  //     exit(0);
  //   }
  // }
  // for (int i = 0; i < x2->dsize(); ++i) {
  //   if (std::isinf(x2->data_[i])) {
  //     std::cout << "before matmul backward has inf." << std::endl;
  //     exit(0);
  //   }
  // }
  // x->to("cuda");
...
__global__ void matmul_backward_kernel(float* x, float* x1, float* x2,
  ...
  if (id_z < z_ && id_y < y_ && id_x < x_) {
    for (int i = 0; i < d_; ++i) {
      ...
      // printf("%.2f %.2f %.2f %d\n", x[x_offset % dsize], x1[(x1_offset + i) % dsize1], x2[(x2_offset + i) % dsize2], x_offset);
    }
  }
}
...
void LayerNorm::forward_process(Tensor& x1, Tensor& x) {
  if (x1.device() == "cpu") {
    ...
    const int block_size = min(
        256, (shape[shape.size() - 1] % 2 == 0 ? shape[shape.size() - 1]
  ...
  if (x1->device() == "cpu") {
    ...
    for (int b = 0; b < x1->dsize() / dim; ++b) {
      ...
      for (int i = 0; i < dim; ++i) {
        ...
        for (int j = 0; j < dim; ++j) {
          x1->grad_[offset + i] += (a1 - ((x1->data_[offset + i] - mean) *
                                          (x1->data_[offset + j] - mean)) /
                                             a2) *
                                   x->grad_[offset + j];
        }
      }
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (offset < dim && head_offset + offset < dsize) {
      ...
      x1_grad[head_offset + offset] +=
          p1 - p2 * (x1_data[head_offset + offset] - mean);
      if (std::isnan(x1_grad[head_offset + offset])) {
        printf("%.2f %.2f %.2f %.2f %.2f\n", stdev, p1, p2, x1_data[head_offset + offset], mean);
      }
    }
  }
}
...
__global__ void before_softmax_kernel(float* x1, index_t dim, index_t sum_size) {
  const int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < sum_size) {
...
void Softmax::before_softmax(Tensor& x1) {
  if (x1.device() == "cpu") {
    ...
    index_t dim = x1.shape()[x1.shape().size() - 1];
    const int block_size = 256;
    const int grid_size = (x1.dsize() / dim + 255) / 256;
    before_softmax_kernel<<<grid_size, block_size>>>(x1.data(), dim, x1.dsize() / dim);
    cudaDeviceSynchronize();
    auto error = cudaGetLastError();
    if (cudaSuccess != error) {
      printf("%s\n", cudaGetErrorString(error));
...
void Softmax::forward_process(Tensor& x1, Tensor& x) {
    ...
    for (int b = 0; b < x1.dsize() / dim; ++b) {
      ...
      for (int i = 0; i < dim; ++i) {
        x[offset + i] = exp(x1[offset + i]) / (exp_sum + 1e-8);
      }
    }
  } else {
    auto shape = x.shape();
    ...
    softmax_kernel<<<grid_size, block_size,
                     sizeof(float) * ((shape[shape.size() - 1] + block_size - 1) / block_size * block_size)>>>(
        x1.data(), x.data(),
        (shape[shape.size() - 1] + block_size - 1) / block_size,
...
__global__ void softmax_kernel(float* x1, float* x, int N, index_t dim,
  ...
  for (int i = 0; i < N; ++i) {
    int offset = thread_idx * N + i;
    if (offset < dim && head_offset + offset < dsize) {
      x[head_offset + offset] = exp(x1[head_offset + offset]) / (exp_sum + 1e-8);
    }
  }
}
...
void Softmax::grad_fn(TensorImplPtr x1, TensorImplPtr x) {
  if (x1->device() == "cpu") {
    ...
    for (int b = 0; b < x1->dsize() / dim; ++b) {
      ...
      for (int i = 0; i < dim; ++i) {
        exp_sum2 +=
            x->grad_[offset + i] * (exp((*x1)[offset + i]) / (exp_sum + 1e-8) / (exp_sum + 1e-8));
      }
      for (int i = 0; i < dim; ++i) {
        x1->grad_[offset + i] += (x->data_[offset + i] * x->grad_[offset + i] -
                                  exp(x1->data_[offset + i]) * exp_sum2);
      }
  ...
  // for (int i = 0;i < x->dsize(); ++i) {
  //               if (std::isnan((*x).grad_[i])) {
  //     std::cout << "before softmax grad has nan. " << i  << std::endl;
  //     exit(0);
  //   }
  // }
  // for (int i = 0;i < x1->dsize(); ++i) {
  //               if (std::isnan((*x1).grad_[i])) {
  //     std::cout << "after softmax grad has nan. " << i  << std::endl;
  //     exit(0);
  //   }
  // }
  // x->to("cuda");
...
__global__ void softmax_backward_kernel(float* x1_grad, float* x1_data,
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (offset < dim) {
      if ((head_offset + offset) < dsize) {
        data[offset] = x_grad[offset + i] * (exp(x1_data[head_offset + offset]) /
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (offset < dim && head_offset + offset < dsize) {
      x1_grad[head_offset + offset] +=
          (x_data[head_offset + offset] * x_grad[head_offset + offset] -
           exp(x1_data[head_offset + offset]) * exp_sum2);
    }
  }
}
...
void NLLLoss::forward_process(Tensor& x1, Tensor& x2, Tensor& x) {
  if (x1.device() == "cpu") {
    ...
    nllloss_kernel<<<grid_size, block_size, sizeof(float) * 256>>>(
  ...
  if (x1->device() == "cpu") {
    for (int i = 0; i < x2->dsize(); ++i) {
      ...
      x1->grad_[offset + (int)(x2->data_[i])] +=
          (-(x->grad_[0]) * x1->data_[offset + (int)(x2->data_[i])]);
    }
  } else {
    const int block_size = 256;
    const int grid_size = (x2->dsize() + 255) / 256;
