...
AdamW::AdamW(detail::ParamsDict decay_params, 
...
__global__ void adamw_kernel(float* grad, float* m1, float* m2, index_t t, float lr, float beta1, float beta2, float ep, float weight_decay, index_t dsize) {
    ...
    if (i < dsize) {
        ...
        grad[i] = -lr_n * (m1[i] / (sqrt(m2[i]) + ep) + weight_decay * grad[i]);    
    }
}
...
void AdamW::step() {
    ...
    for (auto& iter : decay_params_) {
        ...
        if (device_ == "cpu") {
            for (int i = 0; i < p.dsize(); ++i) {
                ...
                m2.get()[i] = beta2_ * m2.get()[i] + (1 - beta2_) * p.grad()[i] * p.grad()[i];
                float lr = lr_ * sqrt(1 - pow(beta2_, t)) / (1 - pow(beta1_, t));
                p.grad()[i] = -lr * (m1.get()[i] / (sqrt(m2.get()[i]) + ep) + weight_decay_ * p.grad()[i]);
            }
        }
        else {
            const int block_size = 256;
            const int grid_size = (p.dsize() + 255) / 256;
            adamw_kernel<<<grid_size, block_size>>>(p.grad(), m1.get(), m2.get(), t, lr_, beta1_, beta2_, ep, weight_decay_, p.dsize());
    ...
    for (auto& iter : nodecay_params_) {
        ...
        if (device_ == "cpu") {
            ...
            const int block_size = 256;
            const int grid_size = (p.dsize() + 255) / 256;
            adamw_kernel<<<grid_size, block_size>>>(p.grad(), m1.get(), m2.get(), t, lr_, beta1_, beta2_, ep, 0, p.dsize());
            cudaDeviceSynchronize();
        }
        p.apply_grad();
    }
